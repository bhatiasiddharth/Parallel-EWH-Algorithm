...
#define NT 4
#define N 4     //No.of of hash tables per process //check this N was 1
#define B 512    //bit size
#define NI 16    //(B/32) no.of integers
#define K 5     //no.of elements in key
#define M (int)pow(2,K)    
#define S (int)(1+K+(K*(K-1)/2))  //error vector hash index size	///check this was 1
...
{
 FILE *fp;
 int i,j,k,l,temp,temp2,n[NI],e=1,index,count,count2=0,co=0,s0=20,num=50,hamdist,myid,nprocs,nlast,rank; 
 int q[NI]={1521,0,0,0,0,1213,1256,123,23,56,67,97,1223,256,4567,1};
 int **table,**ptr,**offset,*score,*data,*erhash,*alpha,*result,*res_dist,*buf;
 double start,end; 
 struct timeval time;
 ...
 //MPI_Scatter(val,K,MPI_INT,val,K,MPI_INT,0,MPI_COMM_WORLD);
 MPI_Bcast(val,K,MPI_INT,0,MPI_COMM_WORLD);  //--Awesome!!
 ...
 for(j=0;j<count;j++)
 {
  ...
  for(i=0;i<N;i++)		///check this definitely
  {
 ...
 //if id=0 Broadcast data and hash functions 
 ...
 */
 ...
 //Select k values from the result
  //free(table);
  buf=(int*)malloc(sizeof(int)*num*nprocs);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Gather (res_dist,num,MPI_INT,buf,num,MPI_INT,0,MPI_COMM_WORLD); 
  for(i=0;i<num*nprocs;i++)
  {
   temp=buf[i];rank=0;
   for(j=0;j<num*nprocs;j++)
     if(temp>buf[j])  rank++;
   if(rank<num)  res_dist[rank]=temp;
  }
